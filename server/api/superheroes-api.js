    ...
    .then(response => {
      ...
      Character.create({
        ...
        Powerstats.create({
          ...
          errorBox(`Problem adding to PowerStats table. ERROR: ${error}`);
          next(error)
        });
      }).catch(error => {
        errorBox(`Problem adding to Character table. ERROR: ${error}`);
        next(error)
      });
      res.send(req.params.id);
    }).catch(error => {
      errorBox(`Problem getting data from Superhero API. ERROR: ${error}`);
      next(error)
    });
  console.groupEnd();
})
